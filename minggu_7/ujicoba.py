        ...
        if submit_button:
            with col1:
                ...
                if max_proba < NEURAL_THRESHOLD:
                    ...
                    sentiment = prediction[0]  # Get the predicted sentiment
                    
                # Display the raw text and prediction
            ...
            with col2:
                ...
                token_sentiment = analyze_sentiment(processed_text, df_combined, model, vectorizer)
                st.write(token_sentiment)

    else:
        st.subheader("About")

# Helper function to convert sentiment into DataFrame
def convert_to_df(sentiment):
    sentiment_dict = {'Sentiment': sentiment}
    sentiment_df = pd.DataFrame(sentiment_dict.items(), columns=['metric', 'value'])
    return sentiment_df

def analyze_sentiment(docx, df_combined, model, vectorizer):
